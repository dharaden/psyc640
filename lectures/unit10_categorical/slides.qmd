...
```{r, highlight=2}
(chi_square = sum((ny_observed - ny_expected)^2/ny_expected)) (critical_val = qchisq(p = 0.95, df = length(ny_expected)-1))
```
...
```{r, highlight=2}
(chi_square = sum((ny_observed - ny_expected)^2/ny_expected)) (critical_val = qchisq(p = 0.95, df = length(ny_expected)-1)) (p_val = pchisq(q = chi_square, df = length(ny_expected)-1, lower.tail = F))
```
...
```{r}
chapekFreq <- xtabs( ~ choice + species, data = chapek9) chapekFreq
```
