...
Then, create a new Quarto Document that we will use for today

Setup the libraries and bring in the data

- We will use the TIPI data from the last lecture that has already been scored
- <a href="../files/data/final_tipi.csv" download="final_tipi.csv">Download Clean TIPI Data (.csv)</a>
...
A quick way to find the mean is to use the aptly named `mean()` function from base R. Use this function to get the average `age` and `hours_of_sleep` in our dataset.
...
Calculate the variance associated with the prices and \# of pieces of LEGO sets.

1\) Go to the [LEGO website](https://www.lego.com/en-us/themes){target="_blank"} and select a Theme.

2\) Record the Price and Number of Pieces for 5 random sets within your theme on the [Google Sheet](https://docs.google.com/spreadsheets/d/1oxGD35Va66iDQvI2H_Sua_UZVEVp2SUya9wclPI6z9s/edit?usp=sharing){target="_blank"}

3\) Calculate the means and variance of each for your Theme
...
```{r}
tipi %>% 
  #filter(duration_in_seconds < 2500) %>% 
...
Define *what* we want to plot (the “mean”), and *how* (as a “bar” chart)

We supply two arguments to the function, `fun =` and `geom =`.
...
❓What if we want to display the median instead of the mean❓
...
❓What about points instead of bars❓
...
Maybe we need some different data

------------------------------------------------------------------------

```{r}
#midwest data - population rates of the midwest
midwest %>% 
  ggplot(aes(state, 
             y = poptotal)) + 
  stat_summary(fun = "mean", 
...
::: callout-note
The order of the layers matters here! Notice how the linerange is on top of the point
:::
...
tipi %>%
  mutate(year_school = as.factor(year_school)) %>% 
  ggplot(aes(x = year_school, 
             y = hours_of_sleep, 
             fill = year_school)) + 
  geom_violin()  +
  labs(title = "Average Hours of Sleep by Year in School", 
       x = "Year in School", 
       y = "# of Hours")
```
