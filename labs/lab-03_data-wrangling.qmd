---
title: "Lab 3: Data Wrangling Beginnings"
...
We have made it to Lab #3! We are going to keep practicing the skills we
started using. We've used pre-installed data before, but we will
continue with some actual participant data. When working with your own
data, you will want to create a workflow of cleaning and wrangling your
data in a reproducible way. These steps will likely occur for every new
dataset that you work with.

Please complete the exercises below. Create a new `Quarto` `.qmd` file
and include the following at the top:
...
When you are finished, be sure to **Render** your document. This will
generate a Word Document, which you will submit along with your `.qmd`
file.
...
::: callout-note
This is the same data that we used in class. If you already have it
downloaded, you do not need to do it again.
:::
...
::: callout-note
You can also use the `janitor` functions that we went over in class

**Be sure to update the "Q85" and "Q85_6_TEXT" to match what is below.**
:::
...
subscales for each of the 5 factors of the Big 5 Personality Index.
Compute the 5 scales. Remember to use the **reverse-scored** items
(`TIPI_2r`), not the original one.s

``` r
# Task 1: Use mutate() to calculate the subscale scores.
# Follow the instructions in the manual to calculate the subscale scores.
...
sheet](https://rstudio.github.io/cheatsheets/data-visualization.pdf){target="_blank"}
...
**Try it out:** Calculate the correlation coefficient for your two
...
gone over this just yet.

**❓Your Answer:**

------------------------------------------------------------------------

*End of Lab. Don't forget to Render to Word* 📄
